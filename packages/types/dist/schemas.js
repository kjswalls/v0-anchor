 ...
 */
const rejectResumeWithDate = (data, ctx) => {
    if (data.paused === false && data.pausedUntil !== undefined) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: 'pausedUntil cannot accompany paused: false — resuming happens now. ' +
 ...
 */
const rejectInvertedRange = (data, ctx) => {
 ...
 */
const rejectProjectForeignKeys = (data, ctx) => {
    if (data.itemIds !== undefined || data.routineIds !== undefined) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: 'projects do not take member arrays — an item names its project, not the ' +
    ...
    if (data.icon !== undefined) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: 'projects wear an `emoji`, not an `icon:<LucideName>` token — that is the ' +
